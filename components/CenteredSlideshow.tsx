...
export default function CenteredSlideshow({ images }: CenteredSlideshowProps) {
        ...
        <div className="flex h-full">
          {images.map((image, index) => {
            ...
            return (
              <div
                key={index}
                className="relative flex min-h-0 min-w-0 flex-[0_0_100%] items-center justify-center"
              >
                {hasDimensions ? (
                  <div className="relative max-h-[60vh] w-full max-w-full" style={{ height: "60vh" }}>
                    <OptimizedImage
                      image={image}
                      alt={`Slide ${index + 1}`}
                      fill
                  ...
                  <div className="h-[60vh] w-full max-w-full bg-[var(--placeholder-color,#f3f4f6)]" />
                )}
              </div>
            );
          })}
